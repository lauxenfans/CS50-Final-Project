...
def save_canvas_as_image():
    ...
    if file_name_png:
        ...
        image = Image.new("RGB", (canvas.winfo_width(), canvas.winfo_height()), color="white")
        ...
        image.paste(ImageGrab.grab(bbox=(x, y, x + width, y + height)))

        # Save the image as a PNG file
        image.save(file_name_png, format="PNG")
        
        tk.messagebox.showinfo("Success", f"Canvas content saved as '{file_name_png}'")
...
#custom color button
custom_b = tk.Button(root, text="Color", command=color_picker)
...
brush_size_b = tk.Scale(root, from_=1, to=30, orient=tk.HORIZONTAL, length=100, command=update_brush_size)
...
random_drawing_b = tk.Button(root, text="Random", command=lambda: generate_random_drawing(canvas))
random_drawing_b.pack(side=tk.LEFT, padx=5, pady=5, fill=tk.BOTH)

# clear button
clear_b = tk.Button(root, text='Clear', command=lambda: clear(canvas))
clear_b.pack(side=tk.LEFT, padx=5, pady=5, fill=tk.BOTH)

# save button
save_button = tk.Button(root, text="Save", command=save_canvas_as_image)
